...
public struct Category: Identifiable
{
    ...
    {
        self.title = title
        addRestaurant(restaurants: restaurants)
    }
    mutating func addRestaurant(restaurants : [Restaurant])
    {
        for restaurant in Restaurant.allRestaurants
        {
            if restaurant.categories.contains(self.title) {
